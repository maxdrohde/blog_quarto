...
- Fit longitudinal ordinal Markov models in a Bayesian framework
...
```{r}
#| echo: true
hist(diff_in_days_benefit, breaks = 100)
```
...
```{r}
#| echo: true
hist(diff_in_mean_time_unwell, breaks = 100)
```

## Timing comparisons

![Comparison of `rmsb` and `blomm` time to fit model](timing.png)
